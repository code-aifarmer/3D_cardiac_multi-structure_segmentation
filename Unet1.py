...
class SpatialAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.layer = nn.Sequential(
            nn.Conv3d(in_channels=in_channels, out_channels=in_channels // 4, kernel_size=1, stride=1, padding=0,
                      bias=False),
            nn.BatchNorm3d(num_features=in_channels // 4),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(in_channels=in_channels // 4, out_channels=in_channels // 4, kernel_size=3, stride=1, padding=1,
                      bias=False),
            nn.BatchNorm3d(num_features=in_channels // 4),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(in_channels=in_channels // 4, out_channels=in_channels, kernel_size=1, stride=1, padding=0,
                      bias=False),
            nn.Sigmoid()
        )
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, root_feat_maps=16, pool_size=2):
        ...
        self.sam = SpatialAttention(in_channels=128)
        self.a = ChannelAttention(in_channels=128)
